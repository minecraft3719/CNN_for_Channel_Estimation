...
cdl = nrCDLChannel; %% nhap thong so
cdl.CarrierFrequency=28e9; %% nhap thong so
cdl.TransmitAntennaArray.Size = [Nt 1 1 1 1]; %% nhap thong so
cdl.ReceiveAntennaArray.Size = [Nr 1 1 1 1]; %% nhap thong so
fc=cdl.CarrierFrequency/1e9; %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% tan so song mang
cdl.MaximumDopplerShift = 0; %% nhap thong so
cdl.ChannelFiltering=false;  %% nhap thong so
%cdl.DelayProfile='Custom';  %%Profile quyet dinh cac tham so
...
% save_PD=cdl.PathDelays;                %%%%Saved data
save_PD=sort(tau-min(tau));
...
% save_AoA=cdl.AnglesAoA;                %%%%Saved Data
